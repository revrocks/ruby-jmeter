...
describe 'tear down thread groups' do
  let(:doc) do
    test do
      teardown_thread_group count: 101, continue_forever: true, duration: 69
    end.to_doc
  end

  let(:fragment) { doc.search("//PostThreadGroup").first }

  it 'should match on num_threads' do
    expect(fragment.search(".//stringProp[@name='ThreadGroup.num_threads']").text).to eq '5'
  end
  ...
  it 'should match on loops' do
    expect(fragment.search(".//stringProp[@name='LoopController.loops']").text).to eq '10'
  end
